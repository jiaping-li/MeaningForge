...
import { MEDICINE_PROTOCOL_VERSION, medicineNarrativeGraphEdges, medicineNarrativeGraphNodes, medicineProtocolCandidates, medicineProtocolChapterAnchors, medicineProtocolReaderPaths, medicineProtocolRelations } from "./medicineProtocol.ts";
...
function protocolNarrativeNodes(title: string) { return title.trim() === "药" ? medicineNarrativeGraphNodes : []; }
function protocolNarrativeEdges(title: string) { return title.trim() === "药" ? medicineNarrativeGraphEdges : []; }
...
export function buildWorkPackage(title: string, source: string, llm: { carriers?: Candidate[]; structural_relations?: RelationCandidate[]; mip_reviews?: MipReview[]; review_notes?: unknown[] } | undefined) {
  ...
  const narrativeGraphNodes = protocolNarrativeNodes(safeTitle).map((node) => {
    const evidence_ids = unique(node.quotes.flatMap((quote) => evidenceForQuote(quote).map((item) => item.id)));
    if (!node.quotes.every((quote) => evidenceForQuote(quote).length > 0)) throw new Error(`Narrative graph node ${node.id} is not fully anchored in the frozen source.`);
    return { id: node.id, label: node.label, kind: node.kind, chapter_id: node.chapter, order: node.order, evidence_ids, description: node.description, provenance_id: "prov-medicine-protocol", review_status: "researcher_checked" as const };
  });
  const narrativeGraphNodeIds = new Set(narrativeGraphNodes.map((node) => node.id));
  const narrativeGraphEdges = protocolNarrativeEdges(safeTitle).map((edge) => {
    const evidence_ids = unique(edge.quotes.flatMap((quote) => evidenceForQuote(quote).map((item) => item.id)));
    if (!narrativeGraphNodeIds.has(edge.source) || !narrativeGraphNodeIds.has(edge.target) || !edge.quotes.every((quote) => evidenceForQuote(quote).length > 0)) throw new Error(`Narrative graph edge ${edge.id} is not fully grounded.`);
    return { id: edge.id, source_id: edge.source, target_id: edge.target, type: edge.type, label: edge.label, evidence_ids, provenance_id: "prov-medicine-protocol", review_status: "researcher_checked" as const };
  });
  const narrative_graph = narrativeGraphNodes.length ? { protocol_version: MEDICINE_PROTOCOL_VERSION, nodes: narrativeGraphNodes, edges: narrativeGraphEdges, lane_order: ["scene", "event", "character", "object", "cue"] as const } : undefined;
