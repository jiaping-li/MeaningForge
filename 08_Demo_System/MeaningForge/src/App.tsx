...
function ThreadWorkspace({ pkg, session, thread, activeRelationId, tab, setTab, evidenceId, onEvidence, onPersist, onClose }: { pkg: WorkPackage; session: ReaderSession; thread: Thread; activeRelationId: string; tab: Tab; setTab: (t: Tab) => void; evidenceId: string; onEvidence: (id: string) => void; onPersist: (fn: (s: ReaderSession) => ReaderSession) => void; onClose: () => void }) {
  ...
  return <section className="thread-workspace"><div className="section-heading"><div><p className="eyebrow">局部推理工作台</p><h2>{activeRelation ? `${nodeLabel(activeRelation.source_id)} ↔ ${nodeLabel(activeRelation.target_id)}` : thread.neutral_label}</h2></div><button className="close" onClick={onClose} title="返回图谱"><X size={15} /></button></div><div className="tabs">{tabs.map(([id, label, Icon]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => { setTab(id); onPersist((s) => event(s, `${id}_open`, activeRelationId || thread.id, activeRelationId ? "structural_relation" : "thread")); }}><Icon size={14} />{label}</button>)}</div>
...
function Trace({ pkg, thread, focusNodeIds, focusEvidenceIds, relations, interpretations, activeEvidence, onEvidence, onPersist }: { pkg: WorkPackage; thread: Thread; focusNodeIds?: string[]; focusEvidenceIds?: string[]; relations: WorkPackage["structural_relations"]; interpretations: WorkPackage["interpretive_relations"]; activeEvidence: string; onEvidence: (id: string) => void; onPersist: (fn: (s: ReaderSession) => ReaderSession) => void }) {
  ...
  return <div className="panel"><p>先看原文证据，再判断这条连接是否值得保留、修改或拒绝。</p><h3>原文证据</h3>{evidenceIds.map((id, n) => <button key={id} className={`evidence-card ${activeEvidence === id ? "active" : ""}`} onClick={() => onEvidence(id)}><b>{n + 1}</b><span>{quote(pkg, id)}</span><small>{find(pkg.evidence, id)?.note}</small></button>)}{mipFeatures.length > 0 && <><h3>MIP 词汇核查</h3>{mipFeatures.map((feature) => <article className="mip-record" key={feature.id}><b>词汇单元：{feature.mip_record?.lexical_unit}</b><p><span>语境义：</span>{feature.mip_record?.contextual_meaning}</p><p><span>基本义：</span>{feature.mip_record?.basic_meaning}</p><p><span>比较：</span>{feature.mip_record?.comparison}</p><small>隐喻候选 · {reviewLabel(feature.mip_record?.review_status)}</small></article>)}</>}<h3>正在检查的连接</h3>{relations.map((r) => <article className="relation" key={r.id}><p>{r.rationale}</p><small>点击上方证据回到原文；你可以在“质疑”中保留、修改或拒绝它。</small></article>)}{interpretations.map((r) => <article className="proposal" key={r.id}><span>可质疑的解释提案</span><p>{r.relation_text}</p><small>{r.qualification}</small><button onClick={() => onPersist((s) => ({ ...event(s, "selection_save", r.id, "interpretive_relation"), selected_evidence_ids: [...new Set([...s.selected_evidence_ids, ...r.evidence_ids])]}))}><Plus size={13} />保存到我的阅读</button></article>)}</div>;
}
...
function Compare({ pkg, thread, evidenceIds, onEvidence }: { pkg: WorkPackage; thread: Thread; evidenceIds?: string[]; onEvidence: (id: string) => void }) {
  ...
  return <div className="panel"><p>比较不是寻找唯一答案，而是让语境中的变化变得可见。</p><div className="compare-selects"><select value={a} onChange={(e) => setA(e.target.value)}>{available.map((id) => <option value={id} key={id}>{find(pkg.evidence, id)?.note}</option>)}</select><select value={b} onChange={(e) => setB(e.target.value)}>{available.map((id) => <option value={id} key={id}>{find(pkg.evidence, id)?.note}</option>)}</select></div><div className="comparison"><EvidenceBlock pkg={pkg} id={a} onOpen={onEvidence} /><EvidenceBlock pkg={pkg} id={b} onOpen={onEvidence} /></div><div className="question">比较它们的场景、谁在说/做什么、前后果，以及该细节在此处是否承担不同作用。</div></div>;
}
