...
function SkeletonTree({ pkg, activeThread, onThread, onEvidence }: { pkg: WorkPackage; activeThread: string; onThread: (id: string) => void; onEvidence: (id: string) => void }) {
  ...
  return <div className="skeleton-tree focus-graph"><div className="tree-root"><BookOpen size={15} /><span>《{pkg.work.title}》</span></div><div className="tree-trunk" /><section className="focus-graph-panel"><div className="graph-level"><span>全文</span><ChevronRight size={11} /><span>{activePath.label}</span></div><div className="cluster-switcher" aria-label="切换阅读路径">{readerPaths.map((path, index) => <button key={path.id} className={index === clusterIndex ? "active" : ""} onClick={() => setFocusedCluster(index)} title={path.label} aria-label={`查看：${path.label}`}>{index + 1}</button>)}</div><p className="path-prompt">{activePath.prompt}</p><div className="network-canvas">{cluster.map((relation) => { const source = positionFor(relation.source_id); const target = positionFor(relation.target_id); return <div className="network-edge" key={relation.id}><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><line x1={source.x} y1={source.y} x2={target.x} y2={target.y} /></svg><button className="edge-label" style={{ left: `${(source.x + target.x) / 2}%`, top: `${(source.y + target.y) / 2}%` }} title="打开这条关系的原文依据" onClick={() => onEvidence(relation.evidence_ids[0])}><BookOpen size={10} /></button></div>; })}{positions.map((position) => <button key={position.id} className={`network-node ${threadForNode(position.id)?.id === activeThread ? "active" : ""}`} style={{ left: `${position.x}%`, top: `${position.y}%` } as CSSProperties} title={`打开“${labelForNode(position.id)}”的证据与工作台`} onClick={() => openNode(position.id)}>{labelForNode(position.id)}</button>)}</div><div className="focus-graph-hint"><span>点击节点展开阅读</span><span>点击连线打开原文</span></div></section></div>;
}
