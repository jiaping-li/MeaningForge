...
function ThreadWorkspace({ pkg, session, thread, activeRelationId, tab, setTab, evidenceId, onEvidence, onPersist, onClose }: { pkg: WorkPackage; session: ReaderSession; thread: Thread; activeRelationId: string; tab: Tab; setTab: (t: Tab) => void; evidenceId: string; onEvidence: (id: string) => void; onPersist: (fn: (s: ReaderSession) => ReaderSession) => void; onClose: () => void }) {
  ...
  return <section className="thread-workspace"><div className="section-heading"><div><p className="eyebrow">局部推理工作台</p><h2>{activeRelationId ? "检查这条连接" : thread.neutral_label}</h2></div><button className="close" onClick={onClose} title="返回图谱"><X size={15} /></button></div><div className="tabs">{tabs.map(([id, label, Icon]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => { setTab(id); onPersist((s) => event(s, `${id}_open`, activeRelationId || thread.id, activeRelationId ? "structural_relation" : "thread")); }}><Icon size={14} />{label}</button>)}</div>
