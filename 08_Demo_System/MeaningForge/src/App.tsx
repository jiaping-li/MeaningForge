...
function SkeletonTree({ pkg, activeThread, onThread, onEvidence }: { pkg: WorkPackage; activeThread: string; onThread: (id: string) => void; onEvidence: (id: string) => void }) {
  ...
  return <div className="skeleton-tree relation-first"><div className="tree-root"><BookOpen size={15} /><span>《{pkg.work.title}》</span><small>全文</small></div><div className="tree-trunk" /><section className="relation-map"><div className="relation-map-heading"><b>可检查的结构路径</b><small>{coreRelations.length ? `${coreRelations.length} 条研究者复核关系` : `${visibleRelations.length} 条候选关系`}</small></div>{visibleRelations.map((relation) => <article className="relation-path" key={relation.id}><div className="relation-path-line"><button className="path-node" onClick={() => openCarrier(relation.source_id)}>{labelForCarrier(relation.source_id)}</button><span className="path-arrow"><i />{relationLabel[relation.type] ?? relation.type}<i /></span><button className="path-node" onClick={() => openCarrier(relation.target_id)}>{labelForCarrier(relation.target_id)}</button></div><p>{relation.rationale}</p><div className="path-actions"><button onClick={() => onEvidence(relation.evidence_ids[0])}>查看原文依据</button><small>{relation.review_status === "researcher_checked" ? "研究者复核 · 仍可质疑" : "自动候选 · 待检查"}</small></div></article>)}</section><section className="carrier-entry"><div><b>从节点进入局部阅读</b><small>节点不是结论；它们是上方关系路径的原文入口。</small></div><div className="carrier-chips">{pkg.carriers.map((carrier) => { const thread = threadForCarrier(carrier.id); return <button key={carrier.id} className={thread?.id === activeThread ? "active" : ""} onClick={() => openCarrier(carrier.id)}>{carrier.label}</button>; })}</div></section></div>;
}
