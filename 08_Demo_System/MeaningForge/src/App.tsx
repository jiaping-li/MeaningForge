...
function SkeletonTree({ pkg, activeThread, onThread, onEvidence }: { pkg: WorkPackage; activeThread: string; onThread: (id: string) => void; onEvidence: (id: string) => void }) {
  ...
  const relationLabel: Record<string, string> = { recurs_with: "回返", co_occurs_with: "共现", shares_scene: "同场", precedes: "先后", contrasts_with: "对照", parallels: "并列" };
  ...
  return <div className="skeleton-tree hierarchy-graph"><div className="tree-root"><BookOpen size={15} /><span>《{pkg.work.title}》</span></div><div className="tree-trunk" /><section className="graph-branches"><div className="graph-legend"><span><i className="legend-node" />可点击节点</span><span><i className="legend-link" />可检查关系</span><small>点击 <BookOpen size={11} /> 回到原文</small></div>{clusters.map((cluster, index) => <article className="graph-cluster" key={cluster.map((relation) => relation.id).join("-")}><span className="cluster-index" aria-label={`结构簇 ${index + 1}`}>{index + 1}</span><div className="cluster-links">{cluster.map((relation) => <div className="cluster-link" key={relation.id}><button className={threadForCarrier(relation.source_id)?.id === activeThread ? "active" : ""} title={`打开“${labelForCarrier(relation.source_id)}”的证据与工作台`} onClick={() => openCarrier(relation.source_id)}>{labelForCarrier(relation.source_id)}</button><span title={relation.rationale}><i />{relationLabel[relation.type] ?? relation.type}<i /></span><button className={threadForCarrier(relation.target_id)?.id === activeThread ? "active" : ""} title={`打开“${labelForCarrier(relation.target_id)}”的证据与工作台`} onClick={() => openCarrier(relation.target_id)}>{labelForCarrier(relation.target_id)}</button><button className="evidence-link" title="查看这条关系的原文依据" aria-label="查看原文依据" onClick={() => onEvidence(relation.evidence_ids[0])}><BookOpen size={11} /></button></div>)}</div></article>)}</section></div>;
}
