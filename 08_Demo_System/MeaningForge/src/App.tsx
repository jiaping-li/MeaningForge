...
function SkeletonTree({ pkg, activeThread, onThread, onEvidence }: { pkg: WorkPackage; activeThread: string; onThread: (id: string) => void; onEvidence: (id: string) => void }) {
  ...
  // UNR relation names are internal taxonomy. The reader sees a compact
  // question/action cue, never raw schema values such as changes_context.
  const relationLabel: Record<string, string> = { recurs_with: "再次出现", co_occurs_with: "一起看", shares_scene: "同一场景", precedes: "前后比较", contrasts_with: "形成对照", parallels: "并列比较", changes_context: "换个语境", changes_function: "作用变化", follows: "前后比较", causal_link: "检查前因后果", consequence_link: "检查后果", shares_actor: "同一人物" };
