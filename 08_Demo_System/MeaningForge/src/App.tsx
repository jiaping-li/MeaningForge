...
function SkeletonTree({ pkg, activeThread, onThread, onEvidence }: { pkg: WorkPackage; activeThread: string; onThread: (id: string) => void; onEvidence: (id: string) => void }) {
  ...
  const clusterIndex = Math.min(focusedCluster, Math.max(0, clusters.length - 1));
  const cluster = clusters[clusterIndex] ?? [];
  const nodeIds = [...new Set(cluster.flatMap((relation) => [relation.source_id, relation.target_id]))];
  ...
  const positionFor = (id: string) => positions.find((position) => position.id === id) ?? positions[0];
  return <div className="skeleton-tree focus-graph"><div className="tree-root"><BookOpen size={15} /><span>《{pkg.work.title}》</span></div><div className="tree-trunk" /><section className="focus-graph-panel"><div className="cluster-switcher" aria-label="切换结构簇">{clusters.map((item, index) => <button key={item.map((relation) => relation.id).join("-")} className={index === clusterIndex ? "active" : ""} onClick={() => setFocusedCluster(index)} title={`查看结构簇 ${index + 1}`} aria-label={`查看结构簇 ${index + 1}`}>{index + 1}</button>)}</div><div className="focus-graph-hint"><span>点击节点展开阅读</span><span>点击关系查看依据</span></div><div className="network-canvas">{cluster.map((relation) => { const source = positionFor(relation.source_id); const target = positionFor(relation.target_id); const x = (source.x + target.x) / 2; const y = (source.y + target.y) / 2; return <div className="network-edge" key={relation.id}><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><line x1={source.x} y1={source.y} x2={target.x} y2={target.y} /></svg><button className="edge-label" style={{ left: `${x}%`, top: `${y}%` }} title={relation.rationale} onClick={() => onEvidence(relation.evidence_ids[0])}>{relationLabel[relation.type] ?? relation.type}</button></div>; })}{positions.map((position) => <button key={position.id} className={`network-node ${threadForCarrier(position.id)?.id === activeThread ? "active" : ""}`} style={{ left: `${position.x}%`, top: `${position.y}%` } as CSSProperties} title={`打开“${labelForCarrier(position.id)}”的证据与工作台`} onClick={() => openCarrier(position.id)}>{labelForCarrier(position.id)}</button>)}</div><div className="focus-evidence">{cluster.map((relation) => <button key={relation.id} onClick={() => onEvidence(relation.evidence_ids[0])} title={relation.rationale}><BookOpen size={12} /><span>{relationLabel[relation.type] ?? relation.type}</span></button>)}</div></section></div>;
}
