  ...
  return <main className="app-shell">
    ...
    <div className="workspace">
      <aside className="left-rail">
        ...
        <section><p className="eyebrow">可生成草稿的原文</p><label className="mip-option"><input type="checkbox" checked={useMIPForDrafts} onChange={(event) => setUseMIPForDrafts(event.target.checked)} />对此处文本运行本地模型的 MIP 辅助核查</label><small>仅在 API 已配置本地模型时生效；结果均为待复核候选。</small>{sourceWorks.map((work) => <button className="work-choice draft-work" key={work.bookUrl} disabled={Boolean(loadingSource)} onClick={() => loadSourceWork(work)}>{loadingSource === work.bookUrl ? <LoaderCircle className="spin" size={16} /> : <BookOpen size={16} />}<span><b>《{work.title}》</b><small>{work.author} · {work.kind} · {useMIPForDrafts ? "MIP 辅助草稿" : "草稿"}</small></span></button>)}{sourceError && <small className="source-error">{sourceError}</small>}</section>
...
function SkeletonTree({ pkg, activeThread, onThread, onEvidence }: { pkg: WorkPackage; activeThread: string; onThread: (id: string) => void; onEvidence: (id: string) => void }) {
  const threadForCarrier = (carrierId: string) => pkg.threads.find((thread) => thread.carrier_ids.includes(carrierId));
  const labelForCarrier = (carrierId: string) => find(pkg.carriers, carrierId)?.label ?? "未命名载体";
  const relationLabel: Record<string, string> = { recurs_with: "词语回返", co_occurs_with: "场景共现", shares_scene: "共享场景", precedes: "文本先后", contrasts_with: "可比较对照", parallels: "并列" };
  const openCarrier = (carrierId: string) => { const thread = threadForCarrier(carrierId); if (thread) onThread(thread.id); };
  ...
  const chapterFor = (relation: typeof visibleRelations[number]) => find(pkg.text_spans, find(pkg.evidence, relation.evidence_ids[0])?.span_ids[0] ?? "")?.chapter_id;
  return <div className="skeleton-tree hierarchy-graph"><div className="tree-root"><BookOpen size={15} /><span>《{pkg.work.title}》</span><small>全文</small></div><div className="tree-trunk" /><section className="graph-branches"><div className="relation-map-heading"><b>全文结构图谱</b><small>{coreRelations.length ? `${coreRelations.length} 条研究者复核关系` : `${visibleRelations.length} 条候选关系`}</small></div>{clusters.map((cluster, index) => { const nodeIds = [...new Set(cluster.flatMap((relation) => [relation.source_id, relation.target_id]))]; const chapterIds = [...new Set(cluster.map(chapterFor).filter(Boolean))]; return <article className="graph-cluster" key={cluster.map((relation) => relation.id).join("-")}><header><span className="cluster-index">{index + 1}</span><div><b>{chapterIds.length ? `第 ${chapterIds.join("、")} 节结构簇` : "可检查结构簇"}</b><small>{nodeIds.length} 个节点 · {cluster.length} 条关系</small></div></header><div className="cluster-nodes">{nodeIds.map((carrierId) => <button key={carrierId} className={threadForCarrier(carrierId)?.id === activeThread ? "active" : ""} onClick={() => openCarrier(carrierId)}>{labelForCarrier(carrierId)}</button>)}</div><div className="cluster-links">{cluster.map((relation) => <div className="cluster-link" key={relation.id}><button onClick={() => openCarrier(relation.source_id)}>{labelForCarrier(relation.source_id)}</button><span><i />{relationLabel[relation.type] ?? relation.type}<i /></span><button onClick={() => openCarrier(relation.target_id)}>{labelForCarrier(relation.target_id)}</button><button className="evidence-link" onClick={() => onEvidence(relation.evidence_ids[0])}>原文</button></div>)}</div><p>{cluster.length === 1 ? cluster[0].rationale : "这些节点通过可回查的结构关系连接；请比较各处语境，而不要把连接当作预设主题。"}</p></article>; })}</section><section className="carrier-entry"><div><b>从节点进入局部阅读</b><small>节点不是结论；它们是图谱路径的原文入口。</small></div><div className="carrier-chips">{pkg.carriers.map((carrier) => { const thread = threadForCarrier(carrier.id); return <button key={carrier.id} className={thread?.id === activeThread ? "active" : ""} onClick={() => openCarrier(carrier.id)}>{carrier.label}</button>; })}</div></section></div>;
}
