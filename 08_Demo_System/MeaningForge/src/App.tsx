...
function MyReading({ pkg, session, claim, setClaim, onEvidence, onPersist }: { pkg: WorkPackage; session: ReaderSession; claim: string; setClaim: (v: string) => void; onEvidence: (id: string) => void; onPersist: (fn: (s: ReaderSession) => ReaderSession) => void }) {
  ...
  return <section className="my-reading"><div className="section-heading"><div><p className="eyebrow">我的阅读</p><h2>我的论证草稿</h2></div><span>{saved.length} 条证据</span></div>
    ...
    <div className="reader-editor"><h3>我的关系</h3><div className="editor-row"><select value={relationSource} onChange={(e) => setRelationSource(e.target.value)}><option value="">起点</option>{nodes.map((node) => <option key={node.id} value={node.id}>{node.label}</option>)}</select><select value={relationTarget} onChange={(e) => setRelationTarget(e.target.value)}><option value="">终点</option>{nodes.map((node) => <option key={node.id} value={node.id}>{node.label}</option>)}</select><input value={relationLabel} onChange={(e) => setRelationLabel(e.target.value)} placeholder="关系" /><button disabled={!relationSource || !relationTarget || relationSource === relationTarget || !relationLabel.trim()} onClick={() => { const id = `reader-relation-${Date.now()}`; onPersist((s) => event({ ...s, reader_relations: [...s.reader_relations, { id, source_id: relationSource, target_id: relationTarget, label: relationLabel.trim(), evidence_ids: saved }] }, "reader_relation_create", id, "reader_relation")); setRelationLabel(""); }}>添加</button></div>{session.reader_relations.map((relation) => <article className="reader-item" key={relation.id}><span>{nodes.find((node) => node.id === relation.source_id)?.label ?? "已删除节点"} → </span><input value={relation.label} onChange={(e) => updateRelation(relation.id, e.target.value)} /><span> → {nodes.find((node) => node.id === relation.target_id)?.label ?? "已删除节点"}</span><button onClick={() => onPersist((s) => event({ ...s, reader_relations: s.reader_relations.filter((item) => item.id !== relation.id) }, "reader_relation_delete", relation.id, "reader_relation"))}>删除</button></article>)}</div>
