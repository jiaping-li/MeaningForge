...
function SpatialTemporalView({ pkg, session, visibleChapterIds, visibleSpanIds, onLinkedSpan, onEvidence, onPersist }: { pkg: WorkPackage; session: ReaderSession; visibleChapterIds: string[]; visibleSpanIds: string[]; onLinkedSpan: (spanId: string) => void; onEvidence: (id: string) => void; onPersist: (fn: (s: ReaderSession) => ReaderSession) => void }) {
  ...
  const sourceEvents = (pkg.narrative_events?.length ? pkg.narrative_events.map((item) => ({ id: item.id, label: item.label, evidenceIds: item.evidence_ids })) : pkg.narrative_units.map((item) => ({ id: item.id, label: item.summary || `叙事单元 ${item.order}`, evidenceIds: pkg.evidence.filter((evidence) => evidence.span_ids.some((spanId) => item.span_ids.includes(spanId))).map((evidence) => evidence.id) }))).filter((item) => item.evidenceIds.some(evidenceVisible)).sort((a, b) => orderFor(a.evidenceIds) - orderFor(b.evidenceIds));
  const personalOrder = session.personal_event_order.filter((id) => sourceEvents.some((item) => item.id === id));
  const events = personalOrder.length ? [...sourceEvents].sort((a, b) => { const ai = personalOrder.indexOf(a.id); const bi = personalOrder.indexOf(b.id); return (ai < 0 ? Number.MAX_SAFE_INTEGER : ai) - (bi < 0 ? Number.MAX_SAFE_INTEGER : bi) || orderFor(a.evidenceIds) - orderFor(b.evidenceIds); }) : sourceEvents;
  const startIndex = Math.floor(events.length * rangeStart / 100);
  const endIndex = Math.max(startIndex + 1, Math.ceil(events.length * rangeEnd / 100));
  const filteredEvents = events.slice(startIndex, endIndex);
...
function MeaningSpace({ pkg, chapter, session: rawSession, researchView, showSignals, showStructure, visibleSpanIds, activeSpanId, onLinkedSpan, onEvidence, onPersist, onOpenMyReading, onBeginInterpretation }: { pkg: WorkPackage; chapter: string; session: ReaderSession; researchView: boolean; showSignals: boolean; showStructure: boolean; visibleSpanIds: string[]; activeSpanId: string; onLinkedSpan: (spanId: string) => void; onEvidence: (id: string) => void; onPersist: (fn: (s: ReaderSession) => ReaderSession) => void; onOpenMyReading: () => void; onBeginInterpretation: (evidenceIds: string[], relationId: string) => void }) {
  ...
  const narrativeLines = (pkg.narrative_graph?.edges ?? []).filter((edge) => (showStructure || edge.type === "precedes") && edge.evidence_ids.some(evidenceInsideHorizon)).flatMap((edge) => {
  ...
  const weakRelations = [...narrativeLines, ...structuralLines];
  ...
  return <section className="meaning-space">
    ...
    <div className="meaning-canvas-layout">
      ...
      {layout !== "reader" && <div className={`reference-relation-status ${showStructure && weakRelations.length ? "has-relations" : "no-relations"}`}>{pkg.narrative_graph ? showStructure ? `${narrativeLines.length} 条叙事关系` : "事件主轴已显示 · 结构按需展开" : referenceRelations.length ? `${referenceRelations.length} 条严格参考关系` : `暂无严格参考关系${candidatePaths.length ? ` · ${candidateItems.length} 个候选路径` : ""}`}</div>}
