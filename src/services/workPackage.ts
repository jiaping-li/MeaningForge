...
export function loadSession(packageId: string, fallback: ReaderSession, scope = "reader"): ReaderSession {
  try {
    ...
    return { ...fallback, ...stored, events, read_chapter_ids: Array.isArray(stored.read_chapter_ids) ? stored.read_chapter_ids : [], read_span_ids: Array.isArray(stored.read_span_ids) ? stored.read_span_ids : [], exposed_candidate_ids: Array.isArray(stored.exposed_candidate_ids) ? stored.exposed_candidate_ids : [], baseline_notes: stored.baseline_notes ?? "", baseline_evidence_references: Array.isArray(stored.baseline_evidence_references) ? stored.baseline_evidence_references : [], study_phase: ["reading", "construction", "final_response", "complete"].includes(stored.study_phase) ? stored.study_phase : fallback.study_phase, phase_history: Array.isArray(stored.phase_history) ? stored.phase_history : [], final_response: stored.final_response ?? "", final_response_evidence_ids: Array.isArray(stored.final_response_evidence_ids) ? stored.final_response_evidence_ids : [], knowledge_cards: Array.isArray(stored.knowledge_cards) ? stored.knowledge_cards : [], reference_reviews: referenceReviews, candidate_decisions: candidateDecisions, counterevidence, reader_evidence_references: readerEvidenceReferences, reader_claims: readerClaims, reader_nodes: legacyNodes, reader_relations: legacyRelations, interpretation_dimensions: Array.isArray(stored.interpretation_dimensions) ? stored.interpretation_dimensions : [], interpretation_ratings: stored.interpretation_ratings && typeof stored.interpretation_ratings === "object" ? stored.interpretation_ratings : {}, personal_event_order: Array.isArray(stored.personal_event_order) ? stored.personal_event_order : [] };
...
export async function downloadSession(session: ReaderSession) {
  ...
  const finalText = session.final_response.trim() || claim?.text.trim() || session.claim.trim() || session.baseline_notes.trim();
  const claimLength = Math.max(1, finalText.length);
...
export async function completeStudySession(session: ReaderSession) {
  await flushStudySync(session, true);
}
