...
export interface CandidateExploration { id: string; label: string; type: string; evidence_ids: string[]; signal_ids: string[]; uncertainty: "candidate" | "undecidable"; prompt: string; context_anchor_ids?: string[]; relation_basis?: string; provenance_id?: string; }
...
export interface ReaderSession {
  package_id: string;
  study_context?: { protocol_version: string; participant_id: string; condition: "meaningforge" | "baseline"; session_id: string; round_id: string; material_id: string; started_at: string };
  read_chapter_ids: string[];
  read_span_ids: string[];
  exposed_candidate_ids: string[];
  ...
  reader_nodes: Array<{ id: string; session_id: string; label: string; type: string; evidence_id?: string; evidence_ids: string[]; source_text_span_id?: string; source_start_char?: number; source_end_char?: number; source_quote?: string; source_candidate_id?: string; interpretation_type?: string; reading_intent?: "keep" | "question"; note?: string; rationale?: string; based_on_node_id?: string; created_at: string; provenance: "reader-authored"; history: Array<{ at: string; action: "create" | "revise"; label: string; type: string; note?: string }> }>;
  ...
  interpretation_ratings: Record<string, Record<string, number>>;
  personal_event_order: string[];
  meaning_map?: { layout: "reference" | "reader" | "compare"; positions: Record<string, [number, number]> };
  events: Array<{ event_id: string; sequence: number; at: string; action: string; target_id?: string; target_type?: string; payload?: Record<string, string | number | boolean>; previous_state?: string; next_state?: string }>;
}
