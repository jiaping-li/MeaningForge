# MeaningForge System Design and Development Specification v7.1

> **Version:** 2026-08-12  
> **Status:** Current implementation authority for the CHI 2027 prototype  
...
It does not need to regenerate the frozen reference scaffold during the main study. A separate preparation command/tool may use deterministic processing and LLM execution to produce the WorkPackage before runtime.
...
Produce a validated, frozen:

```text
Medicine_Substrate_v2_study_ready.json
```

using the fixed Annotation Guide v2. Deterministic code and LLM execution may perform the preparation steps; full manual annotation is not required.
...
Phase-1 substrate follows a fixed protocol; deterministic tools, LLMs, or humans may execute preparation steps.
```

---

# 23. Source Document Status After v7.1
...
```text
      ...
      ┌───────────────────────┼────────────────────────┐
      │                       │                        │
 deterministic code        LLM executor         human (optional)
      │                       │                        │
      └───────────────────────┼────────────────────────┘
                              ↓
                    Narrative Backbone
              ...
              qualified InterpretiveRelation
                              ↓
                    neutral FigurativeThread
                              ↓
              schema + grounding validation
                              ↓
                   FROZEN REFERENCE WORKPACKAGE
                              │
══════════════════════════════╪══════════════════════════════
                              │
                         RUNTIME SIDE
...
The central v7.1 decision is:

> **MeaningForge fixes the substrate ontology, theory-informed processing rules, relation taxonomy, and validation contract. LLMs may replace manual labor when executing those preparation steps, but the generated scaffold remains a contestable reference proposal. The HCI research object is the reader's interaction with that scaffold and the reader's construction of their own evidence-grounded interpretation.**
